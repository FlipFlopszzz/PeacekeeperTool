...
class HomePage(QWidget):
  def __init__(self):
    ...
    self.browser = MarkdownTextBrowser(self.get_md_text())
...
''')
  ...
  def retranslate_ui(self):
    self.browser.text_browser.setMarkdown(self.get_md_text())
...
class SkinPage(QWidget):
  def __init__(self):
    ...
    self.input_box = SingleLineInput(self.tr("输入解密出的5个字母"))
    self.text_display = SingleLineTextDisplay(self.tr("这里将会显示最终的那串字母"))
...
''')
  ...
  def retranslate_ui(self):
    self.browser.text_browser.setMarkdown(self.get_md_text())
    self.audioMorseDecoder.retranslate_ui()
    self.input_box.setText("")
    self.input_box.setPlaceholderText(self.tr("输入解密出的5个字母"))
    self.text_display.setText(self.tr("这里将会显示最终的那串字母"))
...
class GoldPage(QWidget):
  def __init__(self):
    ...
    # 创建输入框
    self.input_box = SingleLineInput(self.tr("请输入解密摩斯电码得到的22个字母"))
    self.input_box.onTextChanged(self.on_text_changed)
    ...
    self.text_display.setText(
        self.tr("这里将会根据8个字母得到8个数字，对应的灯 状态以及开关序列"))  # 显示8行默认文本
    layout.addSpacing(40)

  def on_text_changed(self, text):
    letters = find_missing_letters(text)
    if (len(text) == 0):
      self.text_display.setText(self.tr("这里将会根据8个字母得到8个数字，对应的灯 状态以及开关序列"))
    elif (len(letters) == 1):
      ...
      last_str = self.tr('可能存在以下')+f'{len(letters)}'+self.tr('种情况:')
      for i, l in enumerate(letters):
        last_str += f'\n\n({i+1}) ' + \
            get_last_gold_text(letters[i])
      self.text_display.setText(last_str)
...
''')
  ...
  def retranslate_ui(self):
    ...
    self.audioMorseDecoder.retranslate_ui()
    self.input_box.setText("")
    self.input_box.setPlaceholderText(self.tr("请输入解密摩斯电码得到的22个字母"))
    self.text_display.setText(self.tr("这里将会根据8个字母得到8个数字，对应的灯 状态以及开关序列"))
...
''')
  ...
  def retranslate_ui(self):
    self.browser.text_browser.setMarkdown(self.get_md_text())
    self.candleDecryptor.retranslate_ui()


class BronzePage(QWidget):
  def __init__(self):
    super().__init__()
    layout = QVBoxLayout(self)
    self.browser = MarkdownTextBrowser(self.get_md_text())
...
''')
  ...
  def retranslate_ui(self):
    self.browser.text_browser.setMarkdown(self.get_md_text())
    self.audioMorseDecoder.retranslate_ui()
    self.cipherDecryptor.retranslate_ui()
