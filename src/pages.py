...
## 服务器选择
到伺服器浏览界面，按顺序点击：重设筛选条件-模式选择征服-地图选择某个彩蛋对应的地图-人数不作限制-游戏规模不作限制，在服务器列表中选择没人的服务器，延迟大无所谓没有影响，官服（Official）也无所谓，做彩蛋不怕碰到挂。注意尽量不要选择硬核服（Hardcore）和其他的一些带限制的服（比如B2BCQ,Snipers Paradise），这些服务器的限制（比如硬核服没有小地图，狙服不给用某些武器配备等）会拖慢或影响某些任务的完成。有些服务器进去之后会显示“需要更多玩家才能开始回合”，这个不会影响彩蛋的完成。此外，如果正在做金牌的彩蛋，进去前先点击伺服器资讯，看一下有没有禁用精英兵，如果禁用了就不会刷新精英兵了。
## 狗牌查看
...
class SkinPage(QWidget):
    def __init__(self):
        ...
        md_text = '''
# 逐步升级（皮肤）
1. 在做这个任务之前确保你已经解锁了M1917机枪也就是担架的任意型号，不然拿了皮肤也没用。选择地图阿尔比恩，德国，飞机，朝前飞找到D点的一个铁塔，地上有很多铁箱子和木箱子，用枪(别用手雷，后边要用)给木箱子打了，总共9个铁箱子，各自互动1次，这次和下次都不需要特定顺序。不能多也不能少，错了直接重开。
