...
class AudioRecorder:
    ...
    def start(self):
        ...
        # print(f"\n开始录制，录制文件将保存为: {os.path.abspath(self.FILENAME)}")
    ...
    def stop(self):
        if self.is_recording:
            ...
            # print(f"文件已保存到: {os.path.abspath(self.FILENAME)}")
...
class AudioAnalyzer:
    ...
    def analyze(self, file_path, low_freq=770, high_freq=810, amplitude_threshold=3.0, dot_threshold=0.03,
        ...
        self.morse_signals, self.morse_text, self.char_gaps, self.word_gaps = self.analyze_morse_signal(
            self.times, self.amplitudes, amplitude_threshold, dot_threshold,
            dash_threshold, char_gap_threshold, word_gap_threshold)
    ...
    def analyze_morse_signal(self, times, amplitudes, amplitude_threshold,
                             dot_threshold, dash_threshold, char_gap_threshold,
                             word_gap_threshold):
        """
        分析信号序列，识别摩斯电码
        规则：
        ...
        # 不再需要使用 split 和 join，因为我们已经在 morse_code 中正确处理了间隔
        morse_text = morse_text.strip()  # 只需要清理首尾的空格

        return morse_signals, morse_text, char_gaps, word_gaps
    ...
    #     plt.plot(self.times, self.amplitudes,
    #              'b-', linewidth=1, label='Signal')
    #     plt.axhline(y=amplitude_threshold, color='r', linestyle='--',
    #                 label=f'Amplitude Threshold ({amplitude_threshold})')

    #     # 先绘制所有单词间隔
    #     for gap_start, gap_end in self.word_gaps:
    #         plt.axvspan(gap_start, gap_end, color='red', alpha=0.1,
    #                     label='Word Gap("   ")' if gap_start == self.word_gaps[0][0] else "")

    #     # 再绘制所有字符间隔
    #     for gap_start, gap_end in self.char_gaps:
    #         plt.axvspan(gap_start, gap_end, color='blue', alpha=0.1,
    #                     label='Char Gap(" ")' if gap_start == self.char_gaps[0][0] else "")

    #     # 最后绘制信号，并设置点、划的图例标签
    #     dot_label_set = False
    #     dash_label_set = False
    #     for start, duration, signal_type in self.morse_signals:
    #         if signal_type == 1:  # 点
    #             plt.axvspan(start, start + duration, color='g', alpha=0.3,
    #                         label='Dot(".")' if not dot_label_set else "")
    #             dot_label_set = True
    #         elif signal_type == 2:  # 划
    #             plt.axvspan(start, start + duration, color='y', alpha=0.3,
    #                         label='Dash("_")' if not dash_label_set else "")
    #             dash_label_set = True

    #     plt.title(
    #         f'{low_freq}-{high_freq}Hz Signal Amplitude vs Time (with Morse Code Detection)')
    #     plt.xlabel('Time (s)')
...
if __name__ == "__main__":
    ...
    audio_file = "C:/Users/ASUS/Documents/Bandicam/bf1_morse.wav"
    analyzer.analyze(audio_file)
    morse_code = analyzer.get_morse_code()
