...
- 演示视频中使用的是小号，因为大号的维和者任务做完了导致铜牌没法进行测试。
...
- 软件目前存在的问题：最大化或者从最小化显示任意pyside6项目(即使是空的)时，都会报关于QPainter的错误，即使我没有直接使用。此外，“查看图像”功能使用matplotlib实现，这个会和Qt的事件循环冲突。鉴于以上两个问题并不对软件的使用产生明显的影响，因此暂时没有处理。
- 代码风格比较混乱，因为最开始的一部分是用ai写的，后边大概了解了pyside6之后才自己写。反正应该没人看XD